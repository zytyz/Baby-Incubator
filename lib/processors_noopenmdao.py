    ...
    def run(self, cam):
        ...
        if L > 10:
            ...
            if gap:
                ...
                if len(self.temp_buffer) < 10:
                    self.temp_buffer.append(self.bpm)
                else :
                    self.init_bpm = sum(self.temp_buffer)/10
                    self.temp = self.init_temp+(self.bpm-self.init_bpm)/10
                    
                    self.temp_buffer = []

                text = "(estimate: %0.1f bpm, %0.1f C)" % (self.bpm, self.temp)
            tsize = 1
